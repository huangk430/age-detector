...
def test_model(X_test):
    ...
    for i in range(steps):
        ...
        batch_predictions = model(batch_images)  # predict with only the image
    ...
    mse = np.mean((predictions.flatten() - true_targets)**2)

    mae = np.mean(np.abs(predictions.flatten() - true_targets))
    ...
    results_df.to_csv('cnn/model_versions/v1/test_results.csv', index=False)
